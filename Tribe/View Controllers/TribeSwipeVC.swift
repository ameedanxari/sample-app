...
class TribeSwipeVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UITableViewDelegate, UITableViewDataSource {
    /**  SWIPE DIRECTIONS  **
     **  Left   - DISLIKE  **
     **  Right  - LIKE     **
     **  Up     - NEXT     **
    ...
    @objc func adjustViews() {
        ...
        if Global.tribeSwipeList.count <= Global.currentTribeMatchable {
            outOfSwipes()
        ...
        let currentProfile = Global.tribeSwipeList[Global.currentTribeMatchable]
        
        tblProfile.reloadData()
        
        var imgURL = currentProfile.picUrl?[0]
    ...
    /**
     Dismisses the view controller when no more matches are available to rate.
     */
    func outOfSwipes() {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if Global.tribeSwipeList.count == 0 {
            //do something if no profiles in the venue
            return 0
        }
        
        var currentProfile:TribeNMatchable!
        if Global.currentTribeMatchable >= Global.tribeSwipeList.count {
            currentProfile = Global.tribeSwipeList.last
        } else {
            currentProfile = Global.tribeSwipeList[Global.currentTribeMatchable]
        }
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if collectionView.tag == 1 {
            ...
            let count = (Global.tribeSwipeList[Global.currentTribeMatchable].picUrl?.count)! > 6 ? 6 : Global.tribeSwipeList[Global.currentTribeMatchable].picUrl?.count
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row == Global.currentProfileImageIndex % 6 {
            ...
            //            cell.imgMain.image = UIImage(named: arrOfImages[indexPath.row % arrOfImages.count] as! String)
    ...
    @objc func draggedView(_ sender:UIPanGestureRecognizer){
        ...
        var currentProfile = Global.tribeSwipeList[Global.currentTribeMatchable]
        ...
        if isHorizontalPan == true {
            ...
            if nextProfile == nil { //if this is the last image, do something
                ...
                var imgURL = nextProfile.picUrl?.first
                if (imgURL?.starts(with: "public"))! {
                    imgURL = imgURL?.replacingOccurrences(of: "public", with: TribeAPI.BaseURL.rawValue)
                }
    ...
    func swipeRightComplete(_ currentProfile:TribeNMatchable!, _ nextProfile:TribeNMatchable!) {
        ...
        ServerManager.Instance.sendTribeJudgement(tribeUserId: Global.currentTribeMatch.addedBy.id, matcherId: currentProfile.id, isLike: 0)
        
        //update indexes, set the same image as img1 and reset position
        Global.currentProfileImageIndex = 0
        Global.currentTribeMatchable += 1
        //        Global.currentTribeMatchable = Global.currentTribeMatchable
    ...
    func swipeLeftComplete(_ currentProfile:TribeNMatchable!, _ nextProfile:TribeNMatchable!) {
        ...
        ServerManager.Instance.sendTribeJudgement(tribeUserId: Global.currentTribeMatch.addedBy.id, matcherId: currentProfile.id, isLike: 1)
