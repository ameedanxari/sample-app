...
class TribeNMatched : NSObject, NSCoding{
    ...
    init(fromDictionary dictionary: [String:Any]){
        ...
        tribe = dictionary["tribe"] as? [AnyObject]
        if let val = dictionary["isInvited"] as? Bool {
            isInvited = val
        } else {
            for user in Global.myTribe.pendingMyTribe {
                if user.added.id == id {
                    isInvited = true
                    break
                }
    ...
    @objc required init(coder aDecoder: NSCoder)
    {
        ...
        tribe = aDecoder.decodeObject(forKey: "tribe") as? [AnyObject]
